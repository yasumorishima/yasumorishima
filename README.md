...
- **What none of that establishes** — each verdict is rank one against five or nine permutation worlds, which is not a p-value, and none is an independent estimate of its own size: sizing saw the numbers before the contracts were frozen, and the third row's shape was chosen after a different one failed. The last five rows are sizings, with no contract and no floor; in two of them the rival arm was designed after seeing the round it rivals, and the quarter the last three report on cannot be acted on at all, because which cells fall in it is fixed by the labels of the window being scored. The last row also withdraws three readings of my own that an audit blocked and that I recomputed before accepting. Event-by-event discrimination has not been demonstrated, and magnitudes are not compared across catalogues, so none of it is a statement about Japan
