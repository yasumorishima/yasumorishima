...
**Active:**
- **SIGNATE NEDO Challenge — Baggage-Loading Optimization** — 3D bin packing for airline ULD containers, hybrid offline+online packing agent (¥15M prize pool, Jul–Oct 2026) — public score 47.69 as of 2026-09-17, scored on fill rate, centre of gravity, stability under shaking and placement of priority/soft cargo — half the episodes still end early, and the run logs say why: the loader builds a flat wall to the ceiling at 20% fill, so the back of the container can no longer be reached from the door. Penalising moves that seal off the rear does place 14% more bags and lifts fill and placement on the leaderboard, but it costs more centre-of-gravity than it wins, so the scored best is still the version that leaves the packing style alone
