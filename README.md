...
- **An empty receive log meant two different things** — after the device was pointed at this server it would connect and then close 15 seconds later with nothing received. Reading the firmware showed two stacked timeouts: it waits 10 seconds for the handshake, and only on success sends its `hello` and waits another 10 for the server's. **A handshake that times out therefore emits no `hello` at all**, so zero received bytes cannot distinguish "never sent" from "sent but lost". To rule out this end, the firmware's handshake was reproduced byte for byte — header order (its map sorts them), `Host` without the port, no extensions or user agent, masked frames — and it passes ([test](https://github.com/yasumorishima/stackchan-lab/blob/main/server/test_firmware_handshake.py)). The server now also sends its `hello` unprompted after a short grace period, which opens the channel even if the device's own `hello` is lost
...
The server now exposes thirteen tools of its own alongside the device MCP tools - weather, USD/JPY, stock indices, BTC/ETH prices, NHK news headlines, JMA earthquake, weather-warning and typhoon feeds, heat-stroke risk (WBGT), train delays, an on-this-day lookup, moon age with sunrise and sunset, and the remaining free-tier quota (counted server-side, since probing confirmed the inference API exposes no usage endpoint). Sources are cross-checked before adoption: Coinbase's BTC-JPY spot quoted 3.6x the price that CoinGecko and Yahoo agreed on, so it was excluded. Two field bugs were also traced and fixed. Pure silence advanced no VAD counter, so a device that streams continuously grew the receive buffer until the OOM killer fired - the buffer now flushes on a hard 15-second cap. And speech that occasionally played in slow motion was not the network or the server pacing (measured healthy at 0.86-0.89 of real time across every logged utterance) but the synthesizer itself: Open JTalk's duration model degrades once a pause-free run exceeds about 30 characters (0.19 s/char at 28 chars to 0.52 at 36, measured), voiced audio stretching rather than silence being inserted, and a comma at a non-word boundary does not repair it. Long runs are now split at particle boundaries and synthesized separately, which returned a 23.6-second reply to 11.4 seconds at natural pace. A follow-up report that the *tail* of a reply still dragged turned out to be a second, unrelated fault. Open JTalk pads roughly 0.41s of silence before and 0.58s after every synthesis; because replies are synthesized one sentence at a time, the shorter the reply the larger the share of silence - measured at 27-56% of the audio, with the last sentence always ending on that 0.58s. Trimming the padding and rejoining with a 0.12s pause took six real replies down by 11.6-38.0% with nothing above amplitude 100 lost. The trim threshold came from measurement rather than a guess: the padding is not digital silence but noise at amplitude 20-50, while weak Japanese consonants sit at 50-100, so 60 is the gap between them. Two findings are worth carrying forward. Neither character count nor mora count predicts the stretching - 44 morae can be healthy while a 24-character run of compound kanji (47 morae) degrades by half - what matters is the length of a single accent phrase. And splitting by morae cut the number 29 into 2 and 9, so the splitter now refuses to cut inside a number and leaves the run long instead: a wrong reading is worse than a slow one. Chasing this also surfaced that six test files pinned their import path to the deployed copy rather than their own directory, so working-tree changes were never the ones being tested.

Each new source was checked before it was trusted rather than after. The moon phase is the clearest case: a mean synodic month alone drifts by up to half a day, so the elongation is computed from the Meeus series instead, and the result lands within 0.08-0.21 days of the four phase times the National Astronomical Observatory of Japan publishes for this period - times taken from the observatory's own almanac, because search results only ever gave the date and inventing the hour would have been fabrication. Sunrise and sunset agree with the published solstice values to within a minute. Two sources also turned out to be narrower than they look: the heat index is only published from late April to late October, so out of season the robot says so rather than reporting a dead source, and the train feed's keyless endpoint carries a single operator, which the reply admits instead of implying the whole network is running. An audit pass then caught something worth generalising - an API key passed as a URL query parameter reaches the log through the HTTP client's exception text, which reproduces exactly as written, so the token is masked before anything is recorded.

The robot now answers questions of its own — weather and USD/JPY — through server-side tools merged into the same function-call array as the device's MCP tools (the rate goes through three keyless free sources, market quote first, because a crypto-derived fallback can drift percent-level from the real market). Running it for real also exposed a leak class worth writing down: **a VAD that only starts counting once speech begins never fires on an always-streaming mic**, so ambient audio accumulated unboundedly and the process was OOM-killed at 7GB RSS — twice — while conversations still "worked" thanks to systemd restarts. The fix counts buffered time regardless of speech and discards speechless flushes without ever touching STT; a probe that streams 18 seconds of silence now proves the buffer is dropped at exactly 15.0s ([test](https://github.com/yasumorishima/stackchan-lab/blob/main/server/test_vad.py) / [probe](https://github.com/yasumorishima/stackchan-lab/blob/main/server/probe_vad_silence.py)).
...
<!-- OSS_STATS_START -->(80 PRs / 43 Merged)<!-- OSS_STATS_END --> across 36 repositories. See [oss-contributions](https://github.com/yasumorishima/oss-contributions) for full details.
