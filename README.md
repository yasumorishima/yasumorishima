...
- **Current axis: the isolated mainshock** — the pooled 0.907 is carried by aftershock sequences (per window the arm beats its climatology by +0.018 on aftershock-driven positives, 39% of them, but by only +0.007 on isolated mainshocks, 61%, and by +0.010 on the stricter 11% with no M≥4 within 50 km in the prior year — that last figure being the one that fails its own interval), so since 2026-09 the work targets shocks that are not aftershocks of anything larger, in a causally-selected global arena of 2,280 cells and 36 windows, on top of a bare climatology rather than the shipped forecast. Two of the floored verdicts there: aiming the readout at *large* mainshocks is worth +0.0027 per window at M6.0, and replacing that fitted readout with a per-cell rate counted from past events alone — no model, no feature columns — is worth +0.0086, and on the same windows exceeds the fitted version by +0.0059. A third, smoothing that per-cell rate toward its neighbours, is worth a further +0.0029. Its first attempt passed the rule on the run and then failed its own frozen check on five of twenty-four checks, because the contract fixed the kernel's separation only as a great-circle distance while the one formula it named sat inside a gate, and the arena's regular lattice puts hundreds of cell pairs within a billionth of a degree of the kernel's hard cutoff; naming the formula, removing the cutoff and measuring the tolerance instead of inheriting it, the retake reproduced the verdict and passed all twenty-seven checks. A fourth did not land: bending that kernel into an ellipse along the local axis of past events loses, the control turned across it gains, and the gain turns out to be indistinguishable from a kernel with no direction in it whose width varies per cell — and when that width was itself pre-registered and judged it failed, +0.00030 per window and positive in half of them where the rule wanted three fifths, though held at a fixed strength rather than chosen it is worth +0.00045 and clears all four conditions. That fixed strength was then judged in its own right, with the contract disclosing up front that the value had been picked after seeing it win, so the headline became a reproduction gate -- it gives the earlier number back in all thirty-three windows at exactly zero difference -- and the new content was the floor. It beats the unrestricted shuffle of the widths in all nine worlds, but against a second floor that holds fixed the one confound the previous contract had named and could not resolve -- shuffling widths only among cells with the same number of neighbours within two degrees, which preserves that association to a part in ten thousand while still moving 99.4% of cells -- it comes second of ten, so the verdict is null and the pre-written reading is that the two explanations remain undecided rather than that the edge property accounts for the gain. Building the rival as an arm rather than as a third floor then showed which of the two is actually needed: a width that never measures elongation at all, set from the neighbour count by an affine rule with two scalars and no per-cell seismic quantity in it, is worth +0.00047 per window against the elongation arm's +0.00045, and the two cannot be told apart. What three rounds pursued as narrowing the kernel where past events lie along a line is matched by narrowing it where the arena has fewer cells nearby -- a statement about the smoother at the edge of the studied region, not about the earth. That last step is a sizing with no contract and no floor, and its rival arms were designed after seeing the round they rival. Measuring what an edge cell's kernel actually does then closed that question. The edge is severe -- the edge half of the arena draws on 0.23 times the interior half's effective number of past events, a cell's own weight in its own estimate more than doubles, and the largest gap in azimuth holding no arena at all is 184 to 203 degrees against 87 to 93 -- but the per-cell width does not repair the rate it was raised to repair: against the fraction of a cell's own positives that turned out to be large isolated mainshocks it is worse on the proper score, and worse at the edge. It does remove part of the error that repeats across windows, and does so by following the elongation rather than merely by narrowing more, being deeper than all nine of the previous round's permutations at the edge and sixth of ten in the interior, but it pays for that in variance, so no total error improves and the word correction is withdrawn. Where the gain does live is fixed by an identity over the negatives: three quarters of them sit in cells carrying no positive that window, where the arm already scores 0.83 and the change fails three of the four conditions, and the remaining quarter sit in cells that do, where the arm scores 0.58 and the change is worth +0.00127 in ninety-seven per cent of windows. So the gain is a reordering of the active cells and not a better estimate of any cell's rate; the frozen prediction said the opposite on both counts, and the reading applied is the one written in advance for that outcome. That round is a sizing too, its nine control worlds are the previous round's own, and the neighbour count is a property of the arena's footprint rather than a physical boundary. Each verdict is rank one against five or nine permutation worlds, which is not a p-value, and none is an independent estimate of its own size: sizing saw the numbers before the contracts were frozen, and the third's shape was chosen after a different one failed. Event-by-event discrimination has not been demonstrated. None of it is a statement about Japan (magnitudes are not compared across catalogues)
...
<!-- OSS_STATS_START -->(123 PRs / 73 Merged)<!-- OSS_STATS_END --> across 49 repositories. See [oss-contributions](https://github.com/yasumorishima/oss-contributions) for full details.
...
| **daisy/MathCAT** | [#720-751](https://github.com/daisy/MathCAT/pull/720) | Japanese speech rules, 17 merged PRs: fractions in Japanese word order, exponents, negative numbers, roots, geometry terms, brackets, large operators, intervals, mathematical vocabulary, elementary functions, accents, symbol readings, function application, nPk permutations, the rule vocabulary and the shared-rule vocabulary |
...
- **5th Air Combat AI Challenge** (SIGNATE, hosted by Japan's Acquisition, Technology & Logistics Agency) — behaviour-decision agents for a four-fighter plus one escorted-aircraft engagement in the organiser's simulator; open division, three-dimensional manoeuvre with per-match randomised aircraft performance. Scored by Glicko-2 rating over agent-vs-agent matches, where a win is worth 0.6 plus up to 0.3 for missile hit rate and up to 0.1 for how quickly it is won (¥1.92M prize pool, to 2026-12-04). Entries stay private until the contest closes, per the competition rules.
