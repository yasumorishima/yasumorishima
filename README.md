...
Companion site for an amateur baseball team, forked from Minami Baseball OB — <!--fn:players-->23<!--/fn-->-player roster · <!--fn:pages-->48<!--/fn--> pages · <!--fn:db_tables-->32<!--/fn--> DB tables · <!--fn:e2e_tests-->19<!--/fn--> e2e tests · <!--fn:cost-->¥0<!--/fn-->/mo running cost (<!--fn:ts_files-->166<!--/fn--> files, <!--fn:loc-->~23300<!--/fn--> LOC). **[Technical Documentation](https://github.com/yasumorishima/yokohama-funnies-docs)**
...
<td>Vessel tracking across the Persian Gulf &amp; Gulf of Oman. Collection, storage and a browser-side <a href="https://yasumorishima.github.io/hormuz-ship-tracker/">map</a> run on GitHub Actions and a Hugging Face dataset — no machine of my own<br><br>⚠️ <b>The AIS feed has no receivers in this water</b> — 12,312 vessels worldwide in 180 s on 2026-09-16 and 9,547 on 2026-09-17, none inside the strait. That collector keeps running in case coverage returns.<br><br>🛰️ <b>Sentinel-1 radar now watches the same water</b>, about once every two days, with no account needed. Checked against the project's own AIS archive: 24 of the 24 vessels AIS placed in scored water were found within 300 m — recall only; precision is not measurable from that data.</td>
</tr>
</table>
...
|--------|-----------|----------|
| **NPB 2026** | First true out-of-sample test (predictions frozen 2026-03-23, scored at 95% of the season): beats a constant floor and last-year persistence, but the Bayes+ML layer is indistinguishable from Marcel alone (hitter OPS MAE +0.00095 ± 0.00086, pitcher ERA −0.024 ± 0.015). Intervals were uniformly too wide (80%/95% coverage .954/.992 hitters, .950/.992 pitchers, against nominal .80/.95). σ is now a function of playing time (merged 2026-09-22), which moves provisional coverage to .840/.947 and .851/.975. 8-yr backtest wOBA MAE .0498 | [JP](https://zenn.dev/shogaku/articles/npb-bayes-integration-production) / [EN](https://dev.to/yasumorishima/adding-bayesian-ensemble-monte-carlo-to-an-npb-prediction-app-58po) |
| **NPB 2021 Backtest** | MAE **10.90W** (corrected 2026-09-22; the repo long printed 10.7W, which its own per-team table does not reproduce) — Yakult & Orix last→champion driven by JP player breakouts, not foreign players | [Repo](https://github.com/yasumorishima/npb-2021-backtest) |
...
| Check | Result |
| --- | --- |
| Suites on the rebuild | **785 checks, 0 failures** — reading the file, placing ink on the page, tables from printed rules, glyph clustering, writing the spreadsheet back out, the end-to-end run, and a command line that refuses to start without the per-form knowledge |
| Do the tests have teeth | every claim is paired with a mutation that must turn it red — **42** so far, plus the naive implementation is run alongside so you watch it fail first |
| Counting the teeth from outside | hand-picked mutations only probe the breakages you thought of. A sweep that edits the source mechanically found **10 of 16 mutants surviving** a suite that was entirely green — including one that disabled the read-back check itself. On the next layer it was **26 of 32**. The sweep now carries **98** mutants with none surviving and none failing to apply |
...
| An all-or-nothing gate that misses in the field | deciding a document's type on whether it carries *any* text at all flips on a single page-number fragment in the footer. The grid then reads as a table and a spreadsheet of **entirely empty cells** ships with no warning — the shape is right, so the reader concludes there was nothing to report this month. Real forms always carry a footer |
| Damage that looks like nothing | one ruling line missed on page two silently merges two columns: a part number and its description land in one cell and the quantity moves one column left. Nothing in the parse reports a problem, so the check has to be that **column counts agree across pages** |
| What the sweep still cannot measure | with the sweep at zero survivors, an adversarial review found **three shipping-stoppers**, and all three were gates that had never been written — a per-form constant silently defaulted, so a page printing seven rows shipped five with a green tick; a declaration that a page is a cover which only silenced the warning while the cover's own table was bound in as the header; and a filename gate with no length rule, so the clean month wrote and the *problem* month failed. A mutation only breaks a line that exists |
| Reviewing the repair | re-reviewing after the fix returned **six more**, two of them created by the fix. The worst: on a console that cannot encode the tick and warning glyphs, printing threw — the spreadsheet was already on disk and the screen said it had not been written, with the exit code colliding with "finished, with warnings". **Failing to report is not failing to do the work** |
| Why the tests could not see it | they replace the output stream with an in-memory buffer, and that object has no encoding at all. The harness had removed the failing dimension from the picture — worth asking, once, what a test scaffold is *not* modelling |
| Equivalent or just unreachable | the sweep called one branch equivalent and I deleted it. It was unreachable only because the length gate did not exist yet; adding that gate turned it back into a branch that fails. **If it cannot fail because another gate is missing, there are two holes, not zero** |
...
- **[Kaggriculture](https://www.kaggle.com/competitions/kaggriculture)** — two-player farm-economy simulation, scored by ladder rating rather than by money ($50K prize pool, to 2026-09-30) — the submitted agent wins 0.91 of 96 paired games against the one it replaced, on a lever the per-product ledger found rather than a tuned one: our first fleece reached the market on day 20 against a strong opponent's day 6. The ladder does not resolve that: a submission's rating is a forty-game walk that freezes when the next one replaces it, and three of ours sit at win rates 0.463, 0.434 and 0.432. Agent and its measurement harness in [`kaggle-competitions/kaggriculture`](https://github.com/yasumorishima/kaggle-competitions/tree/main/kaggriculture)
