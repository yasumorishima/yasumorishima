...
- **Every round, with its own caveats** — [RESEARCH-LOG.md](https://github.com/yasumorishima/japan-geohazard-monitor/blob/master/RESEARCH-LOG.md). Most recent rounds are sizings with no floor, several lose outright, and a pass is rank one against five to nineteen permutation worlds, which is not a p-value
- **Open data & automation** — features published as a public Hugging Face dataset, weekly CI pipeline on GitHub Actions
- **Co-seismic InSAR** — 2026 Kumamoto M7.1 from open Sentinel-1 on ASF HyP3: line-of-sight displacement −21.7 to +15.0 cm and a coherence-change damage proxy ([method, figures and caveats](https://github.com/yasumorishima/japan-geohazard-monitor/tree/master/research/kumamoto2026_insar))
...
| **[NPB Foreign Player Study](https://github.com/yasumorishima/npb-foreign-statcast)** | Pre-registered: does MLB Statcast process data (xwOBA, whiff, CSW) predict a foreign player's first NPB season better than MLB results? First run null; 2026 arrivals frozen before the season ends | [Repo](https://github.com/yasumorishima/npb-foreign-statcast) |
...
| **[MLB Data Pipeline](https://github.com/yasumorishima/mlb-data-pipeline)** | Public Hugging Face dataset with a card stating per table whether it is fresh. 14 tables refresh weekly: 12 from Savant and 2 from the MLB Stats API (wOBA, wRC+, FIP, WAR). The 3 FanGraphs tables are frozen at 2025 (the runner's address is blocked). Each run audits what it produced, never publishes a table that lost a season, and goes red otherwise | [HF Dataset](https://huggingface.co/datasets/yasumorishima/mlb-stats) |
...
- **[Kaggriculture](https://www.kaggle.com/competitions/kaggriculture)** — two-player farm-economy simulation, scored by ladder rating rather than by money ($50K prize pool, to 2026-09-30) · agent and measurement harness in [`kaggle-competitions/kaggriculture`](https://github.com/yasumorishima/kaggle-competitions/tree/main/kaggriculture)
