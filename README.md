...
**Amateur Baseball Scorebook Reader** — reads handwritten Japanese paper scorebooks (紙スコアブック) from photos into structured at-bat data. No paid APIs, no cloud OCR: deterministic computer vision (OpenCV on a Raspberry Pi 5) fused with a game-logic constraint solver that only accepts readings consistent with legal base-running. Recognition of the hardest mark class went from 54% (template matching alone) to **82% pooled** with the constraint layer, measured on **25 fully hand-transcribed ground-truth sheets (the complete archive)** — and every sheet transcribed after the model was frozen doubles as a held-out generalization test first (twenty-three consecutive held-out sheets so far, some of them read perfectly). The 2026 season batting stats of my team are compiled from this ground truth. The repository is private (the method is the product) — public technical write-up: [baseball-scorebook-ocr-docs](https://github.com/yasumorishima/baseball-scorebook-ocr-docs) (JP).
...
<!-- KAGGLE_DS_STATS_START -->27 published MLB datasets<!-- KAGGLE_DS_STATS_END -->
