...
- **Current axis: the isolated mainshock** — the pooled 0.907 is carried by aftershock sequences (the arm beats its climatology by +0.018 on aftershock-driven positives, +0.010 on quiet ones, the latter inside its own interval), so since 2026-09 the work targets shocks that are not aftershocks of anything larger, in a causally-selected global arena of 2,280 cells and 36 windows. Two floored verdicts so far: aiming the readout at *large* mainshocks is worth +0.0027 per window at M6.0, and replacing that fitted readout with a per-cell rate counted from past events alone — no model, no feature columns — is worth +0.0086 and beats the fitted version by +0.0059. A within-cell shuffle still produces a stronger arm than the truth, so what is shown is a geographic map rather than event-by-event discrimination, and none of it is a statement about Japan (magnitudes are not compared across catalogues)
...
<!-- OSS_STATS_START -->(120 PRs / 70 Merged)<!-- OSS_STATS_END --> across 49 repositories. See [oss-contributions](https://github.com/yasumorishima/oss-contributions) for full details.
...
| **daisy/MathCAT** | [#720-735](https://github.com/daisy/MathCAT/pull/720) | Japanese speech rules, 12 merged PRs: fractions in Japanese word order, exponents, negative numbers, roots, geometry terms, brackets, large operators, intervals, mathematical vocabulary, elementary functions, accents and symbol readings |
