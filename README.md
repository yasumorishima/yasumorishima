...
| Build the first term that is not fixed per cell inside a window, from event depth, an observable the reader had always dropped *(sizing; the first run stopped at its own gate; a design audit and a post-run audit, both reproduced against the data before being accepted)* | +0.00094 | not a depth term. The gate caught trailing sums built as differences of float32 cumulative sums; re-cut as a direct sum of eleven bins, the check passes 19 of 19. The window-constant depth map then passes the four conditions in no reading at any grid point, and the within-window column gives +0.00094050 causally with all four - but the same field with the deep flag deleted reaches +0.00116444 under the same rule, and projecting that direction out leaves +0.00029472, which fails. Its own floors then settled it: a circular shift, a cross-cell exchange inside strata and a permutation in time all beat the real column in 8 windows of 8 -- as do the time permutations of all eight within-window columns the arena holds, in 128 comparisons of 128; a later shape column that did beat its floor turned out to be matched by the row's index inside the window, which carries no content at all -- and the permutation carries no future, so the true within-window alignment is anti-predictive; the gain lives in a cell-window level that averages over later rows |

- **What none of that establishes** — each pass is rank one against five, nine or nineteen permutation worlds, which is not a p-value, and none is an independent estimate of its own size: sizing saw the numbers before the contracts were frozen, and the third row's shape was chosen after a different one failed. The seven rows before the last three are sizings, with no contract and no floor; in two of them the rival arm was designed after seeing the round it rivals, and the quarter the last five of those report on cannot be acted on at all, because which cells fall in it is fixed by the labels of the window being scored. The four rows before the last two withdraw readings of my own that audits blocked and that I recomputed before accepting; in the two before the last three the blocking faults were all in the design, before any number existed, and in the one before the last three a post-run audit then narrowed a pass rather than blocking it. The row two before the last is a verdict on that sizing, and every window it uses had already been looked at. The row before the last is the first test of that increment on windows never looked at for it, and it fails at a power that was low and measured in advance, so it does not show the increment is absent. The last row is a sizing with no floor whose declared observable was taken off it by its own post-run audit, so it reports a within-window activity term and not depth. Event-by-event discrimination has not been demonstrated, and magnitudes are not compared across catalogues, so no global row is a statement about Japan
...
**Active:**
- **SIGNATE NEDO Challenge — Baggage-Loading Optimization** — 3D bin packing for airline ULD containers, hybrid offline+online packing agent (¥15M prize pool, Jul–Oct 2026) — public score 50.31 as of 2026-09-18, scored on fill rate, centre of gravity, stability under shaking and placement of priority/soft cargo — penalising moves that seal off the unreachable back of the container had looked like a losing trade: it places more bags but costs more centre-of-gravity than it wins. Restricting that penalty to one of the three task types and measuring the mirror version split the effect in two — on the offline-ordering task it is worth +2.62 with all five scored components improving, on the two online tasks it is worth −4.21 — and taking the better half of each lifted the best score by +2.62. The scored components turn out to be exactly additive across task types, to three decimals on all five, so any task-conditional mix of two already-measured versions can now be priced without spending one of the five daily submissions
